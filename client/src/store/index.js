...
const hmy = new Harmony(process.env.VUE_APP_URL_CHAIN, {
...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    setPriceONEonUSD(state, price) {
      state.priceONEonUSD = price;
    }
  },
  actions: {
    ...
    async initMarket({ commit }) {
      let marketAddress = Market.networks[process.env.VUE_APP_CHAIN_ID].address;
    ...
    async fetchPriceONEonUSD({ commit, state }) {
      let market = state.market;
      if (market) {
        let resPrice = await market.methods.getLatestPrice().call(options);
        let price = parseInt(resPrice.toString()) / 10 ** 8;
        commit('setPriceONEonUSD', price);
      }
    }
  },
  modules: {}
